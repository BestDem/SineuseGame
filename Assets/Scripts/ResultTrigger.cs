    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "Player")
        {
            ...
            for (int i = 1; i <= tries; i++)
            {
                ...
                canvasResult.transform.GetChild(i).gameObject.transform.GetChild(0).GetComponent<UnityEngine.UI.Text>().text = PlayerPrefs.GetFloat("Level_" + level + "_accuracy_" + i, 0).ToString("F2") + "%";
            }
        }
    }
